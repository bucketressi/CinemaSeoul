...
public class AdminController {
    ...
    //관리자 수정
    @PutMapping("")
    public void updateAdmi(@RequestBody final AdminVo adv)throws Exception{
        try{
            int result = adminService.updateAdmi(adv);
            if(result == 0){
    ...
    //관리자 조회
    @GetMapping("/{admi_id}")
    public ResponseEntity<AdminInfoDto> selectById(@PathVariable int admi_id){
    ...
    public void deleteUser(@PathVariable int admi_id)throws Exception{
        try {
