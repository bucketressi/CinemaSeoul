...
const MypageInfo = ({ mode, userInfo, fetchUserInfo }: Props) => {
	...
	const updateInfo = () => {
		...
		axios.put(`${SERVER_URL}/user/update`, {
			...
			"email": userInfo.email, //"sample@naver.com",
			"password": password,
	...
	const deleteUser = () => {
		if (!confirm("정말 회원탈퇴 하실 건가요? 다시 되돌릴 수 없습니다."))
			return;
		axios.delete(`${SERVER_URL}/user/delete/${userId}`, {
			headers: {
				"TOKEN": AUTH_TOKEN
			}
		})
	...
	return (
		<>
			<div className="mypage-info-con">
				<div className="form-con">
					<div className="input-con">
						<div>
							<TextField label="이메일" variant="outlined" InputLabelProps={{shrink:true}} value={userInfo?.email} disabled={true}/>
						</div>
			...
			<div className="delete-btn-con">
				<Button className="btn" variant="contained" color="secondary" onClick={deleteUser}>회원탈퇴</Button>
			</div>
		</>
	);
}
